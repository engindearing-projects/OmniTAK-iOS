...
- [~] **GAP-081** CSR enrollment (port 8446) — Android `CsrEnrollment.kt` lands the full flow: fetch CA config → generate RSA-2048 key pair + PKCS#10 CSR → POST with Bearer/Basic/token fallbacks → parse signed cert + CA chain → assemble PKCS#12 → save through `CertVault` → add server with auto-connect. Wired into `MainActivity.handleImportIntent` Enroll branch (replaces the v0.3.0 vc30 stub). **Code shipped — awaiting SxS on a connected device with a live OpenTAKserver.**
...
- [~] **GAP-112** Closed testers shouldn't have to manually configure a server on first launch. `DataPackageBootstrap.runIfNeeded()` (Android: wired in `OmniTAKApp.onCreate()`; iOS: wired in `OmniTAKMobileApp.init()` via a Swift port of the same pattern with a `demo-package.zip.imported` sentinel) copies the bundled demo zip into the importer on first run, seeding `tak.engindearing.soy:8089:ssl` plus the bundled `engie-*.p12` cert + truststore. No UI tap required. Known caveat: shared identity until per-device CSR (GAP-081) is used. **Shipped both platforms** — awaiting SxS on iOS.
...
- [~] **GAP-108** Server-pushed app config / data package settings — new `configBundleUrl` user pref (both platforms). When set, every app launch hits the URL and applies the response: a TAK `.zip` data package goes through the existing importer; a `text/plain` body of `tak://com.atakmap.app/preference?…` URLs (one per line, `#` comments allowed) writes the preferences directly. Operator pattern: publish one URL per team on the OpenTAKserver onboarding portal, every EUD points at it once via QR or Settings, edits server-side propagate on each subsequent launch. Lives at `domain/ConfigBundleFetcher.kt` on Android (wired in `OmniTAKApp.onCreate()`) and `Features/Networking/Services/ConfigBundleFetcher.swift` on iOS (wired in `OmniTAKMobileApp.init()`). Pull-on-launch only; periodic background polling deferred. Closes mata's 80-node airsoft event ask. **Code shipped both platforms — awaiting SxS.**

...
- [ ] **GAP-097** Per-server Marti/Mission REST API port: iOS `TAKServer.secureAPIPort` (nil = 8443) feeds `TAKAPIConfiguration`, carried by `ProfileServer`, editable as "Marti API Port" in the server edit sheet (iOS #114, reported by zeidlos for an OpenTAK host with 8443 taken). Android `TakRestApiClient.SECURE_API_PORT` is still a constant; needs the same field on `TakServer` + Add/Edit Server UI. Android #199
...
### P14: Peer mesh (phone-to-phone CoT)
- [~] **GAP-125** Ditto peer-to-peer CoT sync: nearby devices exchange full CoT XML over BLE / peer-to-peer Wi-Fi / LAN — no TAK server, no extra hardware. Paired implementation: iOS `DittoMeshService.swift` + `DittoMeshSettingsView.swift`, Android `DittoMeshService.kt` + `DittoWire.kt` + `DittoMeshScreen.kt`; Tools-launcher entry on both. Wire contract byte-identical (collection `cot`, schema v1, uid-keyed state, per-message chat keys, staleAt eviction), locked by Android `DittoWireTest` (11 JVM tests). SDK pinned EXACTLY 5.1.0 on both platforms — bump together, never separately (a skewed peer shows as "visible, never syncs"; surfaced via the presence isCompatible count). Off by default — the enable toggle is the consent gate on both platforms; inert without build credentials (Config.xcconfig / local.properties; write DITTO_URL as a bare host — xcconfig eats `//`); separate mesh→server gateway toggle relays original XML with isRelay loop protection. Verified live 2026-08-31: iOS sim + Pixel 6a + Nexus 5X on one mesh — iOS→Android (ALPHA-1 painted on the Pixel map) and Android→iOS (iOS Received counter) over LAN + cloud relay; BLE/AWDL still needs a physical iPhone. **Code shipped both platforms, SxS captured.**

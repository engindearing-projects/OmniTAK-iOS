...
- [ ] **GAP-124** Mesh direct messages — node row + node-detail sheet have a "Message" action that jumps into the Chat tab on a `MESH-DM-{nodeIdHex}` conversation. RX path detects directed packets (`packet.to == myNodeNum`) and buckets them as DMs ("DM: {senderCallsign}") instead of channel chat; broadcasts still bucket by channel. TX path sets `MeshPacket.to` to the recipient's nodenum. RX echo of own outgoing sends is skipped via `packet.from == myNodeNum`. *Shipped on Android — iOS pending.*

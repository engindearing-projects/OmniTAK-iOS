#!/usr/bin/env ruby
# One-off: register AtakDeepLinkExtensions.swift in the OmniTAKMobile
# target so the `/import` and `/preference` subpath handlers compile.
# Idempotent.
require 'xcodeproj'
...
features.path = nil
features.source_tree = '<group>'

networking = features['Networking'] || features.new_group('Networking', nil)
networking.path = nil
networking.source_tree = '<group>'

services = networking['Services'] || networking.new_group('Services', nil)
services.path = nil
services.source_tree = '<group>'

rel_path = 'OmniTAKMobile/Features/Networking/Services/AtakDeepLinkExtensions.swift'
